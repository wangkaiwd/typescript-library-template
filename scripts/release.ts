...
const run = (cmd: string, args: string[], options: Options) => execa(cmd, args, { stdio: 'inherit', ...options });

const ifDryRun = (cmd: string, args: any[], options: Options = {}) => dry ? console.log(`${cmd} ${args.join(' ')}`) : run(cmd, args, options);
